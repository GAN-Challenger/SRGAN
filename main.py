...
def train():
    ...
    with tf.name_scope('g_loss'):
        ...
        tf.summary.scalar('pixel_mse_loss', mse_loss)
        tf.summary.scalar('vgg_loss', vgg_loss)
        tf.summary.scalar('overall_g_loss', g_loss)
    ...
    logger.info('sample HR sub-image: %s, %s, %s' % (sample_imgs_384.shape, sample_imgs_384.min(), sample_imgs_384.max()))
    sample_imgs_96 = tl.prepro.threading_data(sample_imgs_384, fn=downsample_fn)
    logger.info('sample LR sub-image: %s, %s, %s' % (sample_imgs_96.shape, sample_imgs_96.min(), sample_imgs_96.max()))
    ...
    for epoch in range(0, n_epoch_init+1):
        ...
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            logger.info("Epoch [%2d/%2d] %4d time: %4.4fs, mse: %.8f " % (epoch, n_epoch_init, n_iter, time.time() - step_time, errM))
            total_mse_loss += errM
            n_iter += 1
    ...
    for epoch in range(0, n_epoch+1):
        ...
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            logger.info("Epoch [%2d/%2d] %4d time: %4.4fs, W_loss: %.8f g_loss: %.8f (mse: %.6f vgg: %.6f adv: %.6f)"
...
def evaluate():
    ...
    logger.info("took: %4.4fs" % (time.time() - start_time))

    logger.info("LR size: %s /  generated HR size: %s" % (size, out.shape)) # LR size: (339, 510, 3) /  gen HR size: (1, 1356, 2040, 3)
    logger.info("[*] save images")
